window.onload = () => {
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('compositionend', (e) => {
            isComposing = false;
            e.target.value = hiraToKana(e.target.value);
            if (clearBtn) clearBtn.style.display = e.target.value.length > 0 ? "block" : "none";
            if (document.getElementById('allHorseGrid')) { applyFilters(); } else { updateDisplay(); }
        });
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', () => {
            ...
            searchInput.focus();
            document.querySelectorAll('.horse-section').forEach(section => { section.style.display = ''; });
    ...
    const performSearch = () => {
        ...
        if (targetCards.length > 0) {
            // 検索時は一旦他のセクションを隠す
            document.querySelectorAll('.horse-section').forEach(section => { section.style.display = 'none'; });
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            ...
            if (searchInput) { searchInput.value = ''; if (clearBtn) clearBtn.style.display = 'none'; }
            applyFilters();
        });
    }
    if (allHorseGrid) applyFilters();
